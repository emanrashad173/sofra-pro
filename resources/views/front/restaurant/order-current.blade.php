              ...
              @foreach($orders as $order)
                <div class="order-current my-5">
                    <div class="row text-center">
                        ...
                        <div class="col mb-4">
                            <button class="btn bg-mncolor mx-3 px-5">{{$order->client->phone}}</button>
                            <a href=" {{url('restaurant/confirm-order/'.$order->id)}}"><button class="btn btn-success mx-3 px-5">تأكيد التسليم</button></a>
                        </div>
                    </div>
                </div>
                @endforeach
